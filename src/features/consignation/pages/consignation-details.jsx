...
useEffect(() => {
  const fetchTypes = async () => {
    ...
    if (consignation && consignation.multi_consignation_id) {
      // For secondary consignations in a group, we need to find the parent ID
      
      // Option 1: If the multi_consignation_id points to the parent
      const parentId = consignation.multi_consignation_id;
      
      // Option 2: If we need to find the first consignation in the group
      // (this would be needed if multi_consignation_id is just a group identifier)
      /*
      ...
      const parentId = firstCons?.id || id;
      */
...
doc.text("• Signature de l'exécuteur :", marginX + 5, ln2);
  ...
  const handleGenerateExactCustomPDF = async () => {
    ...
    } else if (consignation.status === "deconsigné" || consignation.status === "archived") {
      try {
        const { data, error } = await supabase
          .from("deconsignations")
          .select(`
        ...
        console.error("Erreur lors de la récupération des détails de déconsignation :", err);
        deconsData = {
          id: "",
          created_at: "",
          demandeur_id: "",
    ...
    // Generate the PDF with all our data, including first consignation and last deconsignation dates
