...
doc.setFontSize(FONT.BODY);
  ...
  // IMPROVED handleGenerateExactCustomPDF function with Capacitor mobile support
    ...
    try {
      ...
      if (doc) {
        ...
        const fileName = consignation.multi_consignation_id 
          ? `consignation-${consignation.multi_consignation_id}-${formatDateForFilename(consignation.date_consignation)}.pdf`
          : `consignation-${id}-${formatDateForFilename(consignation.date_consignation)}.pdf`;
        ...
        if (Capacitor.isNativePlatform()) {
          ...
          try {
            ...
            try {
              ...
              await Share.share({
                title: fileName,
                text: `Consignation ${consignation.multi_consignation_id || id}`,
                url: savedFile.uri,
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <ButtonSpinner
                  ...
                  className="flex items-center justify-center gap-2 px-4 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-purple-600 disabled:hover:shadow-md transition-all font-medium shadow-md hover:shadow-lg"
                  loading={isAddingMultiConsignation}
                  loadingText="Redirection..."
                  disabled={consignation.status === "archived"}
                >
                  <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
