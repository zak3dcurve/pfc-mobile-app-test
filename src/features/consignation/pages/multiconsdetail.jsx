...
// Add this useEffect to load deconsignation data immediately
useEffect(() => {
    if (id) {
    ...
    useEffect(() => {
        const fetchDecons = async () => {
            ...
            console.log("Decons:", JSON.stringify(data));
            
            // Also fetch blended deconsignations data
    ...
    // In MulticonsDetail.jsx, update the fetchBlendedDeconsignations function:
const fetchBlendedDeconsignations = async () => {
    try {
        ...
        if (!deconsRecords || deconsRecords.length === 0) {
            console.log("No deconsignation records found");
    ...
    const handleMultiDetailsButton = (consignationId) => {
        navigate(`/consignationdetails/${consignationId}`);
    }

    const handleAddConsignation = () => {
        navigate(`/multiconsignation/${id}`);
    }
    ...
    const generateExactFormPDF = (consData, deconsData, typesData, groupedConsignations = null, blendedDeconsData = null) => {
        try {
            ...
            const execConsDate = formatDateTimePDF(
                firstConsignationDate || consData.created_at
            );
            ...
            const execDeconDate = formatDateTimePDF(
                ...
                blendedDeconsData && blendedDeconsData.length > 0 ? 
      ...
      const zoneName = safeGet(consData, 'zones.name', "");
            ...
            doc.addImage(
                ...
                "PNG", marginX + 5, currentY - 15, 90, 30);
...
// Or even safer, you can use the safeGet helper function you already have:
const deconsignatorNameSafe = deconsRows.length > 0 
    ? safeGet(deconsRows[0], 'deconsignateur.name', executant)
    : executant;
...
const deconsignatorName = deconsRows.length > 0 && deconsRows[0]?.deconsignateur?.name 
...
doc.setFontSize(FONT.BODY);
    ...
    //////////// fetch all consignations 



    const fetchAllConsignationsForPDF = async () => {
    const { data, error } = await supabase
        .from("consignations")
        .select(`
            *,
        ...
        `)
        .eq("multi_consignation_id", id)
        // No status filter here - get ALL consignations
        .order("created_at", { ascending: false });

    if (error) {
        console.error("Error fetching all consignations for PDF:", error);
        return [];
    }
    
    console.log("All consignations for PDF:", data);
    return data || [];
};
...
export default MulticonsDetail;