...
useEffect(() => {
  const fetchDecons = async () => {
    ...
    console.log("Consi:", JSON.stringify(data));
  };
  fetchDecons();
}, [id]);
...
const [multiConsignationId, setMultiConsignationId] = useState(null);
const [filteredConsignations, setFilteredConsignations] = useState([]);
...
const handleConfirmation = async () => {
  ...
  const handlePrint = () => {
    ...
    // No real déconsData in a multi‑block—pass an empty object
    const deconsData        = deconsignations[0] || {};
  
    // Flatten all types into the same shape your single‑row generator expects
    const typesData = consignations.flatMap((c) =>
      (c.consignation_types_junction || []).map((j) => ({
        type: { id: null, type_name: j.types_consignation.type_name }
      }))
    );
  
    // groupedConsignations → all your rows
    const groupedConsignations = consignations;
     const groupedDeconsData = deconsignations;
...
doc.setFontSize(10);
    ...
    doc.setFontSize(10);
    doc.text(
      `Déconsignation demandée le : ${deconsData && deconsData.created_at ? formatDateTimePDF(deconsData.created_at) : "//___ à ___ h"}`,
      marginX + 5,
      deconsBlockY + 15
    );
