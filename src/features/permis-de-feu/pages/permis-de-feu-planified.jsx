...
const PermisDeFeuPlanified = () => {
  ...
  const fetchPlanifiedPermis = async () => {
    ...
    if (permisError || !permisData) {
      console.error("Error fetching planified permis:", permisError);
      setLoading(false);
      return;
    }
  ...
  const columns = [
    ...
    },
    {
      name: "Lieu",
      selector: (row) =>
        row.zones && row.zones.name ? row.zones.name : "Non défini",
      sortable: true,
    },
    {
      name: "Nom responsable",
      selector: (row) =>
        row.responsables && row.responsables.name ? row.responsables.name : "Non défini",
      sortable: true,
    },
    {
      name: "Description",
...
const handleRowClick = (row) => {
  // Navigate to edit page for planified permits
